...
/* Data size is 4 + 128 + 4 + 1 = 137 bytes. We pad to 140 bytes as this is the nearest divisible by
   4 we have. So we add 3 bytes */
...
static TResult assemble(char *outputBuffer, const char *inputBuffer, int len) {
    ...
    for (i = 0; i < bytesLeft; i++) outputBuffer[counter++] = inputBuffer[i];
...
TResult deserialize(const char *buffer, int len, void *output) {
    ...
    if (result == PACKET_COMPLETE) {
        ...
        for (i = 0; i < comms->dataSize; i++) checksum ^= comms->buffer[i];
...
int serialize(char *buffer, void *dataStructure, size_t size) {
    ...
    for (i = 0; i < size; i++) checksum ^= comms.buffer[i];
